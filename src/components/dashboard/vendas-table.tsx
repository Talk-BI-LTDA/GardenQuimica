// src/components/dashboard/vendas-table.tsx
'use client'
...
export function VendasTable({ 
  ...
  const aplicarFiltros = async () => {
    ...
    try {
      ...
      if (resultadoNaoVendas.success) {
        // Aqui está a correção - usar resultadoNaoVendas.naoVendas em vez de processedNaoVendas
  ...
  const editarItem = (item: Venda | NaoVenda, tipo: 'venda' | 'naoVenda') => {
    if (tipo === 'venda') {
      router.push(`/dashboard/vendas/${item.id}/editar`);
    } else {
      router.push(`/dashboard/nao-vendas/${item.id}/editar`);
    }
  };
